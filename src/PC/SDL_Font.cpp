...
void SDL_Font::init() {
  ...
  _surface_buffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 1280, 720, 32, rmask,
                                         gmask, bmask, amask);
...
void SDL_Font::loadfont(const unsigned int size) {
  ...
  ASSERT_FUNC((font != NULL),
              printf("TTF_OpenFont error:  %s/n", TTF_GetError()));
...
void SDL_Font::shutdown() {
  ...
  for (std::map<unsigned int, TTF_Font*>::iterator it = _fonts.begin();
       it != _fonts.end(); ++it) {
...
void SDL_Font::apply_surface(int x, int y, SDL_Surface* source,
  ...
  // sdlTexture =
  // SDL_CreateTextureFromSurface(Engine::OGL::OGL_Renderer::getSDLRenderer(),
  // source);
  // Blit

  SDL_BlitSurface(source, clip, destination, &offset);
  // SDL_RenderCopy(Engine::OGL::OGL_Renderer::getSDLRenderer(), sdlTexture,
  // clip, NULL);
}

//! Renders a single character to the specified buffer at the specified position
void SDL_Font::render(const int buffer, const uint32_t character, const float X,
                      const float Y) {
  ...
  ASSERT_FUNC((text_surface != NULL),
              printf("TTF_RenderGlyph_Solid:  %s/n", TTF_GetError()));
...
void SDL_Font::renderString(const int buffer, const char* characters,
                            const float X, const float Y) {
  SDL_Surface* text_surface;

  text_surface = TTF_RenderText_Blended(getFont(_fontSize), characters, _color);
  ASSERT_FUNC((text_surface != NULL),
              printf("TTF_RenderText_Solid:  %s/n", TTF_GetError()));
...
void SDL_Font::setSize(float Size) { _fontSize = (int)Size; }
