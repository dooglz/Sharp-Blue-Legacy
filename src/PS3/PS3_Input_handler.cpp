...
void PS3_Input::Destroy() { cellPadEnd(); }
...
void PS3_Input::Update() {
  ...
  if (data.len > 0) // Has Pad status changed ?
  {
    ...
    for (int i = 0; i < PADBUTTONS_MAX / 2; ++i) {
      Input::setValue(ctrler + padbuttons_names[i],
                      255 * ((bool)(data.button[2] & (1 << i))));
    }
    // data.button [3] : More Digital button statuses
    for (int i = 0; i < PADBUTTONS_MAX / 2; ++i) {
      Input::setValue(ctrler + padbuttons_names[i + 8],
                      255 * ((bool)(data.button[3] & (1 << i))));
    }
    ...
    //-- Right stick
    Input::setValue(ctrler + analogue_names[ALOG_RIGHT_STICK_X],
                    (unsigned char)(data.button[ALOG_RIGHT_STICK_X + 4]));
    Input::setValue(ctrler + analogue_names[ALOG_RIGHT_STICK_Y],
                    (unsigned char)(data.button[ALOG_RIGHT_STICK_Y + 4]));

    //-- Left stick
    Input::setValue(ctrler + analogue_names[ALOG_LEFT_STICK_X],
                    (unsigned char)(data.button[ALOG_LEFT_STICK_X + 4]));
    Input::setValue(ctrler + analogue_names[ALOG_LEFT_STICK_Y],
                    (unsigned char)(data.button[ALOG_LEFT_STICK_Y + 4]));
