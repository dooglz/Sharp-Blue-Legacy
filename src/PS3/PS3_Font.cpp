...
void PS3_Font::init() {
  ...
  ASSERT_FUNC((err == 0),
              printf("CELL_SYSMODULE_FS module failed! error: %x\n", err));

  err = cellSysmoduleLoadModule(CELL_SYSMODULE_FONT); // libfont(font rendering)
  ASSERT_FUNC((err == 0),
              printf("CELL_SYSMODULE_FONT module failed! error: %x\n", err));

  err = cellSysmoduleLoadModule(
      CELL_SYSMODULE_FREETYPE); // FreeType2 (low-level font access)
  ASSERT_FUNC(
      (err == 0),
      printf("CELL_SYSMODULE_FREETYPE module failed! error: %x\n", err));

  err = cellSysmoduleLoadModule(
      CELL_SYSMODULE_FONTFT); // Interface between libfont and FreeType2
  ASSERT_FUNC((err == 0),
              printf("CELL_SYSMODULE_FONTFT module failed ! error: %x\n", err));
  ...
  config.userFontEntrys =
      &UserFontEntrys[0]; // Starting address for custom font storage
  config.userFontEntryMax = 0; // No custom fonts, only system fonts
  err = cellFontInit(&config); // Initialize libfont
  ASSERT_FUNC((err == 0),
              printf("Initialize libfont module failed! error: %x\n", err));
  ...
  ASSERT_FUNC((err == 0),
              printf(" cellFontInitLibraryFreeType failed! error: %x\n", err));
  ...
  ASSERT_FUNC((err == 0),
              printf("cellFontOpenFontset failed! error: %x\n", err));
  ...
  err = sys_memory_allocate(1024 * 1024, SYS_MEMORY_PAGE_SIZE_1M,
                            (sys_addr_t*)(int)&fontGcmConfigPool);
  ASSERT_FUNC((err == 0),
              printf("sys_memory_allocate failed! error: %x\n", err));
  ASSERT_FUNC((fontGcmConfigPool != NULL),
              printf("error mallocing fontGcmConfigPool\n"));
  // Align the memory
  fontGcmLocalConfigPool =
      GCM::GCM_MemoryUtils::localMemoryAlign(128, 128 * 1024);
  ASSERT_FUNC((fontGcmLocalConfigPool != NULL),
              printf("error aligning fontGcmLocalConfigPool\n"));
  // Map the Main memory pool, so the RSX can use it.
  uint32_t offset;
  err = cellGcmMapMainMemory(fontGcmConfigPool, 1024 * 1024, &offset);
  ASSERT_FUNC((err == 0),
              printf("cellGcmMapMainMemory failed! error: %x\n", err));
  ...
  ASSERT_FUNC((err == 0),
              printf("cellFontInitGraphicsGcm failed! error: %x\n", err));
  ...
  ASSERT_FUNC(
      (err == 0),
      printf("cellFontGraphicsSetupDrawContext failed! error: %x\n", err));

  // Link the GCM font Renderer to our front and back buffers
  err = cellFontRenderSurfaceGcmInit(&FontRenderSurfaces[0],
                                     &Engine::GCM::GCM_Renderer::_surfaces[0]);
  ASSERT_FUNC((err == 0),
              printf("cellFontRenderSurfaceGcmInit failed! error: %x\n", err));
  err = cellFontRenderSurfaceGcmInit(&FontRenderSurfaces[1],
                                     &Engine::GCM::GCM_Renderer::_surfaces[1]);
  ASSERT_FUNC((err == 0),
              printf("cellFontRenderSurfaceGcmInit failed! error: %x\n", err));
...
void PS3_Font::renderString(const int buffer, const char* characters,
  ...
  for (int i = 0; i < length; i++) {
    ...
    float rescaleX = (w * 1.0); // Depends on cellFontSetScalePixel() being
                                // 1.0f.
...
void PS3_Font::render(int buffer, uint32_t character, float X, float Y) {
  int err = cellFontGraphicsGcmSetDrawGlyph(
      gCellGcmCurrentContext, &FontRenderSurfaces[buffer], X, Y,
      &find(character)->VertexesGlyph, &drawContext);
  ASSERT_FUNC(
      (err == 0),
      printf("cellFontGraphicsGcmSetDrawGlyph failed! error: %x\n", err));
}
...
void PS3_Font::loadCharacter(uint32_t code) {
  ...
  ASSERT_FUNC((err == 0),
              printf("cellFontGenerateCharGlyph failed! error: %x\n", err));

  // Get Outline Control Distance
  err = cellFontGlyphGetOutlineControlDistance(
      glyphA, maxScale, CELL_FONT_GLYPH_OUTLINE_CONTROL_DISTANCE_DEFAULT,
      &distance);
  ASSERT_FUNC(
      (err == 0),
      printf("cellFontGlyphGetOutlineControlDistance failed! error: %x\n",
             err));

  // Generate VertexesGlyphs
  err = cellFontGlyphSetupVertexesGlyph(glyphA, distance, fontVertexBuffer,
                                        fontVertexBufferSize, &VertexesGlyphA,
                                        &dataSize);
  ASSERT_FUNC(
      (err == 0),
      printf("cellFontGlyphSetupVertexesGlyph failed! error: %x\n", err));
  ...
  ASSERT_FUNC((err == 0),
              printf("cellFontGetCharGlyphMetrics failed! error: %x\n", err));
