...
void GCM_VertexShader::SetDefaultAttributes() {
  // GetParameter() could be used, but would put the parameter result in the
  // map, which could be unnecessary.
  ...
  // Get the index of the vertex attribute that will be set for the vertex
  // shader
  // Turn parameters into resources, These are used for
  // cellGcmSetVertexDataArray();
  if (position_param) {
    VERTEX_POSITION_INDEX =
        cellGcmCgGetParameterResource(program, position_param) - CG_ATTR0;
  }

  if (colour_param) {
    VERTEX_COLOUR_INDEX =
        cellGcmCgGetParameterResource(program, colour_param) - CG_ATTR0;
  }
}
}
}