...
void GCM_Renderer::init() {
  ...
  int err = 0;
  err = cellGcmInit(COMMAND_SIZE, BUFFER_SIZE,
                    memalign(1024 * 1024, BUFFER_SIZE));
...
void GCM_Renderer::InitDisplay() {
  ...
  // Set the video configuration, we haven't changed anything other than
  // possibly the Z/colour depth
...
void GCM_Renderer::InitSurfaces() {
  ...
  // Allocate a 64byte aligned segment of RSX memory that is the size of a depth
  // buffer
  void* depthBuffer = GCM_MemoryUtils::localMemoryAlign(64, depthSize);
  uint32_t depthOffset;

  /* cellGcmAddressToOffset converts an effective address in the area accessible
     by the RSX to an offset value.
          An offset is the space between from the base address of local memory
     and a certain useable address.
          Offsets are used in gcm commands that deal with shader parameters,
  ...
  for (int i = 0; i < _numberOfSurfaces; ++i) {
    /// Allocate a 64byte aligned segment of RSX memory that is the size of a
    /// colour buffer
    ...
    Parameters:
    cellGcmSetDisplayBuffer ( Buffer ID (0 - 7), memory offset, pitch -
    Horizontal byte width,
    width - Horizontal resolution (number of pixels), height - Vertical
    resolution(number of pixels)
    */
    cellGcmSetDisplayBuffer(i, _surfaces[i].colorOffset[0], color_pitch,
    ...
    // Format of the depth and stencil buffers (16-bit depth or 24-bit depth and
    // 8-bit stencil)
  ...
  /*
  The surfaces[] array contains CellGcmSurface objects and is in stack memory
  somewhere,
  and a bunch of new buffer objects have just been created and stored in RSX
  Local Memory.
  Each CellGcmSurface object has a pointer to its corresponding buffer in
  .colorOffset [0].
  When we call cellGcmSetSurface(), we pass it an CellGcmSurface from our array,
  The parameters that we set on that object will be read, processed and passed
  to the RSX.
  */
...
void GCM_Renderer::SetCurrentShader(GCM_VertexShader& vert,
...
void GCM_Renderer::renderMesh(stMesh* msh, Matrix4 mvp) {
  ...
  cell::Gcm::cellGcmSetVertexDataArray(
      msh->vertShader->VERTEX_POSITION_INDEX, // index
      0, // Frequency
  ...
  cell::Gcm::cellGcmSetVertexDataArray(
      msh->vertShader->VERTEX_COLOUR_INDEX, 0, sizeof(stVertex), 4,
  ...
  if (msh->line == true) {
    if (msh->strip == true) {
      cell::Gcm::cellGcmSetDrawArrays(CELL_GCM_PRIMITIVE_LINE_STRIP, 0,
                                      msh->numVerts);
    } else {
      cell::Gcm::cellGcmSetDrawArrays(CELL_GCM_PRIMITIVE_LINES, 0,
                                      msh->numVerts);
    }
  } else {
    if (msh->fan == true) {
      cell::Gcm::cellGcmSetDrawArrays(CELL_GCM_PRIMITIVE_TRIANGLE_FAN, 0,
                                      msh->numVerts);
    } else if (msh->strip == true) {
      cell::Gcm::cellGcmSetDrawArrays(CELL_GCM_PRIMITIVE_TRIANGLE_STRIP, 0,
                                      msh->numVerts);
    } else {
      cell::Gcm::cellGcmSetDrawArrays(CELL_GCM_PRIMITIVE_TRIANGLES, 0,
                                      msh->numVerts);
    }
  }
}
...
unsigned char fs_basic[208] = {
  
};
...
unsigned char vs_basic[672] = {
  
};
}
}