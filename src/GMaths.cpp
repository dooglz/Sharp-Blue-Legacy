...
Matrix4 rotation(const Vector3 rotation) {
  Matrix4 a = Matrix4::rotation(float(rotation.getX() * (M_PI / 180.0f)),
                                Vector3(1, 0, 0));
  a *= Matrix4::rotation(float(rotation.getY() * (M_PI / 180.0f)),
                         Vector3(0, 1, 0));
  a *= Matrix4::rotation(float(rotation.getZ() * (M_PI / 180.0f)),
                         Vector3(0, 0, 1));
  return a;
}
...
Matrix4 perspective(const float FoV, const float ratio, const float nearClip,
                    const float farClip) {
...
Matrix4 rotation(const Vector3 rotation) {
  Matrix4 a = glm::rotate(glm::mat4(1.0f), float(rotation.x * (M_PI / 180.0f)),
                          Vector3(1, 0, 0));
  a *= glm::rotate(glm::mat4(1.0f), float(rotation.y * (M_PI / 180.0f)),
                   Vector3(0, 1, 0));
  a *= glm::rotate(glm::mat4(1.0f), float(rotation.z * (M_PI / 180.0f)),
                   Vector3(0, 0, 1));
  return a;
}
...
Matrix4 perspective(const float FoV, const float ratio, const float nearClip,
                    const float farClip) {
  return glm::perspective(float(FoV * (M_PI / 180.0f)), ratio, nearClip,
                          farClip);
}

Matrix4 identity() { return glm::mat4(1.0f); }
...
Quaternion FromAxisAngle(const Vector3 &v, float angle) {
  ...
  return Quaternion(normVector.x * sinAngle, normVector.y * sinAngle,
                    normVector.z * sinAngle, cos(angle));
}
...
namespace M3 {
Vector3 cross(const Vector3 a, const Vector3 b) { return glm::cross(a, b); }
}
...
Vector2 getFrustrumBoundsAtDepth(float depth, float farX, float farY,
                                 float farZ, float nearX, float nearY) {
  float A = 0, X = 0, Y = 0;
  A = ((farZ - depth) / farZ);
  X = A * farX + ((1 - A) * nearX);
...
bool withinFrustrum(Vector3 position) { return true; }
}
...
// only translate
Vector3 MatrixToEuler(const Matrix4 m) { return Vector3(m[3]); }
