	...
	namespace GXM{
		...
		void GXM_Shader::registerWithPatcher(){
			...
			for(uint32_t i=0; i < (params);++i)
			{
				...
				//insert into map
				_params.insert( std::pair<std::string, const SceGxmProgramParameter*>(pramName,p) );
			}
		}

		const SceGxmProgramParameter * GXM_Shader::getParameter(const std::string name)
		{
			std::map<std::string, const SceGxmProgramParameter*>::iterator it = _params.find(name);
			if (it != _params.end()) {
				//already in map
				return (it->second);
			}
