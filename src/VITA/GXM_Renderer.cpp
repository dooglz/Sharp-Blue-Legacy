...
namespace Engine{
	namespace GXM{
		...
		const SceGxmProgram *const GXM_Renderer::clearVertexProgramGxp = &_binary_clear_v_gxp_start;
		const SceGxmProgram *const GXM_Renderer::clearFragmentProgramGxp = &_binary_clear_f_gxp_start;
		const SceGxmProgram *const GXM_Renderer::basicVertexProgramGxp = &_binary_basic_v_gxp_start;
		const SceGxmProgram *const GXM_Renderer::basicFragmentProgramGxp = &_binary_basic_f_gxp_start;
		...
		void GXM_Renderer::InitSurfaces()
		{
			...
			for (uint32_t i = 0; i < DISPLAY_BUFFER_COUNT; ++i) {
				...
				for (uint32_t y = 0; y < DISPLAY_HEIGHT; ++y) {
					uint32_t *row = (uint32_t *)_surfacesData[i] + y*DISPLAY_STRIDE_IN_PIXELS;
		...
		void GXM_Renderer::clearSurface() {
			...
			sceGxmDraw(_context, SCE_GXM_PRIMITIVE_TRIANGLES, SCE_GXM_INDEX_FORMAT_U16, clearIndices, 3);
			...
			sceGxmSetUniformDataF(vertexDefaultBuffer, basicVertShader->getParameter("wvp"), 0, 16, wvpData);

			// draw the spinning triangle
			sceGxmSetVertexStream(_context, 0, basicVertices);
			sceGxmDraw(_context, SCE_GXM_PRIMITIVE_TRIANGLES, SCE_GXM_INDEX_FORMAT_U16, basicIndices, 3);
		...
		void GXM_Renderer::swapBuffers()
		{
			...
			sceGxmPadHeartbeat(&_surfaces[backBufferIndex], _surfacesSync[backBufferIndex]);
		...
		void GXM_Renderer::SetCurrentShader(SceGxmVertexProgram & vert, SceGxmFragmentProgram & frag)
		{
		...
		void  GXM_Renderer::loadDefaultShaders()
		{
			...
			blankFragShader = new GXM_Fragment_Shader("clearFragment",clearFragmentProgramGxp);
			...
			basicFragShader = new GXM_Fragment_Shader("basicFragment",basicFragmentProgramGxp);
			...
			clearVertexAttributes[0].regIndex = sceGxmProgramParameterGetResourceIndex(blankVertShader->getParameter("aPosition"));
			...
			blankFragShader->createPatch(SCE_GXM_OUTPUT_REGISTER_FORMAT_UCHAR4, MSAA_MODE, NULL,blankVertShader->unpatchedProgram);
			...
			//The clear vertex & index data is static, we allocate and write the data here.
			...
			basicVertexAttributes[0].regIndex = sceGxmProgramParameterGetResourceIndex(basicVertShader->getParameter("aPosition"));
			...
			basicVertexAttributes[1].regIndex = sceGxmProgramParameterGetResourceIndex(basicVertShader->getParameter("aColor"));
			...
			basicFragShader->createPatch(SCE_GXM_OUTPUT_REGISTER_FORMAT_UCHAR4, MSAA_MODE, NULL,basicVertShader->unpatchedProgram);
