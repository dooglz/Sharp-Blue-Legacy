...
inline std::vector<stVertex> CreateTorus(float InnerRadius, float OuterRadius,
  ...
  ASSERT_MSG((InnerRadius >= 0),
             "InnerRadius can not be negative"
             "(well, you can but than it is no longer a donut)");
  ASSERT_MSG((OuterRadius > InnerRadius),
             "OuterRadius must be bigger than InnerRadius");
  ...
  for (unsigned int i = 0; i < Rings; i++) {
    const float curRings[2] = { stepRing * (i + 0), stepRing * (i + 1) };
